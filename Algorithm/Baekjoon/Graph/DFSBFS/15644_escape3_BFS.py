...
def bfs(rx: int, ry: int, bx: int, by: int, depth: int, save_direction: str) -> None:
    global result
    global result_direction
    for i in range(len(dx)):
        ...
        if escape_map[next_rx][next_ry] == 4:
            result = depth
            result_direction = save_direction + direction[i]
            return
        ...
        if is_visited[next_rx][next_ry][next_bx][next_by] == 0:
            ...
            q.append((next_rx, next_ry, next_bx, next_by, depth + 1, save_direction + direction[i]))

max_count: int = 10
result: int = -1
result_direction: str = ''
while q:
    red_x, red_y, blue_x, blue_y, now_depth, now_direction = q.popleft()
    if now_depth > 10:
        break

    bfs(red_x, red_y, blue_x, blue_y, now_depth, now_direction)
    if result != -1:
        break
print(f'{result}')
if result != -1:
    print(f'{result_direction}')